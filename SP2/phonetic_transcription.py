...
def read_input_file(filename: Path):
    ...
    with open(filename, 'r', encoding='utf-8-sig') as file:
        for line in file:
            element = line.split(';')
            key = element[0].strip()
            sentence = element[1].strip()

            sentences_dict[key] = sentence
    file.close()
...
def apply_basic_rules(sentences_dict: dict, basic_rules: dict):
    res = {}
    for key in sentences_dict:
        sentence = sentences_dict[key]
        pause = '$ '
...
def apply_alophones(sentences_dict: dict):
    # alophones = ['n', 'm', 'R', 'r', 'l']
...
def apply_assimilation(sentences_dict: dict):
    res = {}
    special_chars = ['#', '!', '$']
    for key in sentences_dict:
        sentence = sentences_dict[key]
...
def check_prepositions(sentences_dict: dict):
    res = {}
